...
def test_run_raises_on_failure(repo):
    with pytest.raises(WorktreeError):
        run_git(["git", "-C", str(repo), "not-a-command"])
...
def test_remove_fallback_when_dir_deleted_out_of_band(manager, repo):
    ...
    out = run_git(["git", "-C", str(repo), "branch", "--list", manager.branch])
    assert out.strip() == ""
...
def test_apply_to_repo_leaves_uncommitted_changes(manager, repo):
    ...
    out = run_git(["git", "-C", str(repo), "status", "--porcelain"])
...
def test_byte_exact_despite_autocrlf(repo, tmp_path):
    ...
    run_git(["git", "-C", str(repo), "config", "core.autocrlf", "true"])
