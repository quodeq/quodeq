...
from quodeq.analysis._report_assembly import ReportData, assemble_report_dict
...
def test_totals_carry_violations_per_100_files():
    report = assemble_report_dict(_data([_violation("R-FT-1", vt="x")] * 3, files_read=8))
    assert report["totals"]["violationsPer100Files"] == 37.5


def test_density_is_none_when_nothing_was_read():
    report = assemble_report_dict(_data([_violation("R-FT-1", vt="x")], files_read=0))
...
def test_unmapped_types_lists_untagged_and_other_by_requirement():
    ...
    report = assemble_report_dict(_data(violations))
...
def test_unmapped_types_is_capped_at_50_rows():
    report = assemble_report_dict(_data([_violation(f"R-{i}") for i in range(60)]))
