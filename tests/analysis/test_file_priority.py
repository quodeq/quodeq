...
class TestComputeGitScores:
    def test_parses_git_log(self, tmp_path):
        ...
        scores = compute_git_scores(
            ["file1.py", "file2.py"], tmp_path, log_source=lambda src, months: iter(mock_lines)
        )
    ...
    def test_git_not_available(self, tmp_path):
        scores = compute_git_scores(["file1.py"], tmp_path, log_source=lambda src, months: iter([]))
        assert scores == {}
    ...
    def test_recent_file_gets_recency_boost(self, tmp_path):
        ...
        scores = compute_git_scores(
            ["recent.py", "old.py"], tmp_path, log_source=lambda src, months: iter(mock_lines)
        )
