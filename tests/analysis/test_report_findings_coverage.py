...
class TestFlattenFindings:
    def test_empty_items(self):
        assert flatten_findings([], "Auth", ("file", "line")) == []
    ...
    def test_tags_with_label_and_keeps_fields(self):
        ...
        result = flatten_findings(items, "Auth", ("file", "line"))
        assert len(result) == 2
    ...
    def test_skips_none_values(self):
        ...
        result = flatten_findings(items, "P1", ("file", "line"))
        assert result == [{"principle": "P1", "file": "a.py"}]

    def test_missing_fields_skipped(self):
        items = [{"file": "a.py"}]
        result = flatten_findings(items, "P1", ("file", "line", "severity"))
        assert result == [{"principle": "P1", "file": "a.py"}]
...
class TestBuildPrincipleRow:
    def test_basic_row(self):
        ...
        row = build_principle_row("auth", pdata, lookup)
    ...
    def test_fallback_display_name(self):
        ...
        lookup = {}
        row = build_principle_row("raw_key", pdata, lookup)
...
class TestBuildPrincipleRows:
    ...
    def test_scope_downgrade_survives_flatten(self):
        # VIOLATION_FIELDS whitelists what flatten_findings keeps when
