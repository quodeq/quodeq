import json
from quodeq.analysis.prompts.builder import load_dimension_data
...
def test_load_dimension_data_from_compiled(tmp_path):
    ...
    result = load_dimension_data(compiled, "security")
...
def test_load_dimension_data_fallback_to_evaluators(tmp_path):
    compiled = tmp_path / "compiled"
    compiled.mkdir()
    result = load_dimension_data(compiled, "nonexistent", evaluators_dir=tmp_path / "evals")
...
def test_load_dimension_data_custom_evaluator(tmp_path):
    ...
    result = load_dimension_data(compiled, "clean-arch", evaluators_dir=evals)
    assert result is not None
