...
class TestEvictCompletedJobs:
    def test_evicts_excess(self):
        ...
        from quodeq.services._job_model import MAX_COMPLETED_JOBS
        # Add more than MAX_COMPLETED_JOBS done jobs
        for i in range(MAX_COMPLETED_JOBS + 5):
            store.put(Job(f"j{i}", "done", [], "now", "later", 0))
        mgr._evict_completed_jobs()
        remaining = store.list()
        assert len(remaining) == MAX_COMPLETED_JOBS
...
class TestEvictionOrder:
    def test_evicts_oldest_completed_first(self):
        """Eviction beyond MAX_COMPLETED_JOBS must drop the OLDEST jobs.
        ...
        """
        from quodeq.services.jobs import MAX_COMPLETED_JOBS

        store = InMemoryJobStore()
        mgr = JobManager(job_store=store)
        total = MAX_COMPLETED_JOBS + 3
