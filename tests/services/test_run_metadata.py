"""``read_run_metadata`` joins status.json and dim_estimates.json.

Coverage percentages already travel on each dimension entry of the dashboard
payload; this adds only what the reports do not carry: the commit and the
cache hit / miss / excluded counts, from two small files.
"""
...
def _seed(run_dir: Path) -> None:
    ...
    (run_dir / "dim_estimates.json").write_text(json.dumps({
        "security": {"count": _MISSES, "total": _TOTAL, "cached": _CACHED, "excluded": _EXCLUDED}}),
        encoding="utf-8")
...
def test_read_run_metadata_joins_sources(tmp_path: Path) -> None:
    ...
    assert meta["cacheStats"]["security"] == {"cached": _CACHED, "misses": _MISSES, "excluded": _EXCLUDED}
