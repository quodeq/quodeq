"""Totals.violations_per_100_files survives report read-back and recount."""
...
def test_parse_totals_reads_density():
    ...
    assert t.violations_per_100_files == 37.5


def test_parse_totals_density_absent_is_none():
    assert parse_totals({"violationCount": 3}).violations_per_100_files is None


def test_recount_totals_recomputes_density():
    v = [Finding(practice_id="FT", verdict="violation", severity="major")] * 3
    assert recount_totals(v, compliance_count=0, files_read=8).violations_per_100_files == 37.5
    assert recount_totals(v, compliance_count=0).violations_per_100_files is None
