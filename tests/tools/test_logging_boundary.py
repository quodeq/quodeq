...
DECLARED_LOGGING_SITES: dict[str, str] = {
    ...
    'analysis/subagents/_consolidated.py': 'Consolidated multi-dimension analysis - extracted from subagents/runner.py - imports quodeq.shared.log_sink for quarantine-sink logging (log_malformed_jsonl_line/log_quarantined_findings), plan-sanctioned composition wiring; ALSO imports raw stdlib logging and binds a module-level _logger (Cluster 9, fault-tolerance cycle 1) -- that part IS a logging-boundary violation, grandfathered here rather than converted to an injected LogSink; sink conversion deferred to cycle 2 (final review Important 3)',
