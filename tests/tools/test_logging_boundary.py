...
DECLARED_LOGGING_SITES: dict[str, str] = {
    ...
    'core/utils/_file_lock.py': "Platform file-locking helpers (fcntl/msvcrt dispatch) - moved inward from data/file_lock.py so api/ can use it per the check_imports layer rule, same as core/utils/io.py; its pre-existing 'import logging' was carried over unchanged, out of scope for this sweep",
    ...
    'services/_fs_project_helpers.py': 'Project-building helpers for the filesystem action provider - Task C6 (usability sweep) added a warning log for a previously-silent malformed-repo-identifier fallback in find_existing_project; out of scope for the SEP-06 sweep (not a flagged per-site conversion)',
