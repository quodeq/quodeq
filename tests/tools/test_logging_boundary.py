...
DECLARED_LOGGING_SITES: dict[str, str] = {
    ...
    'analysis/cache/failure_streak.py': 'Consecutive-failure circuit breaker for the dim runner - out of scope for this sweep (not a flagged per-site conversion)',
    ...
    'analysis/subagents/_consolidated.py': 'Consolidated multi-dimension analysis - extracted from subagents/runner.py - imports quodeq.shared.log_sink for quarantine-sink logging (log_malformed_jsonl_line/log_quarantined_findings), plan-sanctioned composition wiring (a raw stdlib logging import added by Cluster 9 of fault-tolerance cycle 1 was converted to the injected LogSink in the same cycle; this file must not import logging)',
    ...
    'services/cache.py': 'Shared LRU cache factory for dimension fetchers - out of scope for this sweep (not a flagged per-site conversion)',
    'services/ephemeral_cleanup.py': 'Lifecycle management for ephemeral clones under ~/.quodeq/clones/ - out of scope for this sweep (not a flagged per-site conversion)',
    ...
    'services/fs_projects.py': 'Project CRUD helpers for the filesystem action provider - out of scope for this sweep (not a flagged per-site conversion)',
    'services/fs_scan.py': 'Quick-scan service: extract project metadata without AI evaluation - out of scope for this sweep (not a flagged per-site conversion)',
    ...
    'services/warmup.py': 'Background warm-up of per-project score caches at server boot - out of scope for this sweep (not a flagged per-site conversion)',
