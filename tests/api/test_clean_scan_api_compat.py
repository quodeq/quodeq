...
def test_clean_scan_field_default_false():
    opts = build_evaluation_options({})
    assert opts.clean_scan is False


def test_clean_scan_field_explicit_true():
    opts = build_evaluation_options({"cleanScan": True})
...
def test_legacy_incremental_false_maps_to_clean_scan_true():
    ...
    with patch.object(_eval_helpers_mod._logger, "warning") as mock_warn:
        opts = build_evaluation_options({"incremental": False})
    assert opts.clean_scan is True
...
def test_legacy_incremental_true_maps_to_clean_scan_false():
    ...
    with patch.object(_eval_helpers_mod._logger, "warning") as mock_warn:
        opts = build_evaluation_options({"incremental": True})
    assert opts.clean_scan is False
    _assert_deprecation_warned(mock_warn)
...
def test_conflicting_fields_rejected():
    ...
    with pytest.raises(ValueError, match="cannot be combined"):
        build_evaluation_options({"incremental": True, "cleanScan": True})
...
def test_conflicting_fields_rejected_even_when_values_align():
    ...
    with pytest.raises(ValueError, match="cannot be combined"):
        ...
        build_evaluation_options({"incremental": True, "cleanScan": False})
