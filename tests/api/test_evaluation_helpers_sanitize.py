"""Tests for sanitize_url in _evaluation_helpers.py.
...
ported here with sanitize_url's own return shape: scheme + "***@" +
host/path, credentials masked rather than dropped).
"""
from __future__ import annotations

from quodeq.api._evaluation_helpers import sanitize_url
...
def test_sanitize_url_simple_user_pass():
    assert sanitize_url("https://user:token@host/repo.git") == "https://***@host/repo.git"


def test_sanitize_url_token_only():
    assert sanitize_url("https://ghp_supersecrettoken@github.com/org/repo.git") == (
