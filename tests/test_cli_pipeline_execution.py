"""Tests for CLI pipeline execution — build_cli_manifest, execute_pipeline, save_manifest."""
...
class TestBuildManifest:
    def test_no_prescan_returns_none(self):
        from quodeq.cli import build_cli_manifest
        args = argparse.Namespace(no_prescan=True)
        result = build_cli_manifest(args, Path("/tmp"), MagicMock())
    ...
    def test_detection_file_missing_returns_none(self):
        ...
        result = build_cli_manifest(args, Path("/tmp"), paths)
        assert result is None
...
class TestExecutePipeline:
    ...
    def test_pipeline_analysis_error(self, mock_run_full, tmp_path, capsys):
        ...
        with pytest.raises(AnalysisError, match="AI failed"):
            execute_pipeline(args, config, evidence_dir, evaluation_dir)
...
class TestSaveManifest:
    ...
    def test_os_error_silenced(self, mock_write, mock_to_dict, tmp_path):
        ...
        save_manifest(manifest, tmp_path)  # should not raise
