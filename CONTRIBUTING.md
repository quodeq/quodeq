...
- Duplication: `npm run lint:clones` fails the build on any block of 5+ lines and 40+ tokens that appears twice in `src/quodeq` (Python and UI; tests and fixtures excluded); the baseline is empty. Extract a helper in the package (`_helpers.py`, a feature helpers module), in `quodeq/shared/` or `core/utils/` across Python packages, or in `src/utils/` across UI features. Two copies that differ in one detail become one helper with a parameter, never a third copy.
- Prose in code says what the code is for, not how it got there. A module docstring or comment that reads "Split from x.py to keep it under 300 lines", "Extracted verbatim", "moved from", "kept so tests can patch" or names a plan, task, cluster or run id describes an edit, and the next reader learns nothing from it; the history lives in git. Write what the module or symbol does. `tests/tools/test_no_planning_labels.py` fails the build on plan and audit labels; `tests/tools/test_no_refactor_narration.py` ratchets refactor narration per site (`tools/refactor_narration_baseline.txt` may only shrink). This applies to code written by agents too: a brief that asks for a split or an extraction must also ask for a real docstring.
