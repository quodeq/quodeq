...
- `data/fs/dimension_report/` builds and persists per-dimension report files (split into `_report_constants.py`, `report_scoring.py`, `_report_findings.py`, `_report_assembly.py`, `report_io.py` for disk persistence); `analysis/report.py` and its sibling `analysis/_report_*.py` modules re-export the package's public names as shims so pre-existing import paths stay live. The reader counterpart lives at `data/fs/report_parser/`.
