...
| `tools/check_fault_tolerance.py` via `tests/tools/test_fault_tolerance_ratchet.py` | no bare `except:`, empty `except: pass`, or non-re-raising `except Exception:`/`except BaseException:` (`src/quodeq`); vendored/generated dirs excluded | `tools/fault_tolerance_baseline.txt` | `python tools/check_fault_tolerance.py --update-baseline` (line-keyed: prefer hand edits) |
