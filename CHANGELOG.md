...
### Changes
- **Subagents launch at full width after the scout**: cloud runs still start one scout agent per dimension to catch entry problems (auth, quota, oversized prompt) before committing to more. Once the scout finishes or has run for 30 seconds, every remaining `--n-subagents` slot launches at once, bounded only by the files left in the queue. Previously the number of extra agents was estimated from the file count, so dimensions with 50 or fewer files ran on a single agent and larger ones rarely reached the configured count.
