...
NOT a violation: literal inside a constant/enum definition; long function that only registers routes; duplicated test setup; code that IS the remediation for the issue; the flagged line or its neighbour already contains the guard (`escapeHtml`, `length` check, null guard, `try`/`catch`); algorithm-intrinsic complexity (force-directed layout, MST, per-frame trig); count equals limit (`max 5` is `> 5`, not `>= 5`); SQL assembled by interpolation when every interpolated part is a module-level constant (a fixed column list) or a generated run of placeholder markers, and every caller value travels in the driver's parameter list, a `PRAGMA` or `SET` value included because those cannot be bound; a generic catch (`except Exception`, `catch (e)`) at a fault-isolation boundary, meaning a thread or task entry point, one iteration of an event loop or work queue, or a third-party or plugin callback, when the handler records the error with its stack trace at warning level or above and does not continue with partially-updated state (a generic catch around ordinary logic, one that logs below warning or drops the stack trace, is still R-FT-7); a generic catch that re-raises the caught exception, or checks its type and rethrows every other one.
