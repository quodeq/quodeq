...
The hash itself lives in ``core.finding_identity`` (it is also the dismiss
identity since #1165); this module re-exports it and keeps the precedent
loading around it.
"""
...
from quodeq.core.finding_identity import fingerprint, normalize_snippet
...
def precedent_text(req: str | None, snippet: str | None) -> str | None:
    ...
    Note: ``.rstrip()`` on top of ``normalize_snippet`` mops up the trailing
    space that punctuation-stripping can leave behind (e.g. "x = 1 ;" ->
    "x = 1 "); it's applied here rather than in ``normalize_snippet``
    itself so ``fingerprint()``'s hash stays byte-for-byte unchanged.
    """
    norm = normalize_snippet(snippet).rstrip()
