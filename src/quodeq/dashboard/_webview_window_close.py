...
def _show_macos_close_alert(result: dict, done: threading.Semaphore) -> None:
    ...
    try:
        ...
        alert.setMessageText_(CLOSE_CONFIRM_TITLE)
...
def _spawn_close_prompt_worker(
    ...
    worker = threading.Thread(
        target=prompt_close_choice_and_finish, args=(api, window, state, job_id), daemon=True,
    )
    worker.start()
    return worker


def _make_on_closing_inline(api: "WindowApi", window: object) -> "Callable[[], bool]":
    ...
    directly rather than via ``ask_close_choice`` so it does not re-dispatch on
...
def _make_on_closing_async(api: "WindowApi", window: object) -> "Callable[[], bool]":
    ...
    (``prompt_close_choice_and_finish``, above). Dict-item writes are
...
def make_on_closing(api: "WindowApi", window: object) -> "Callable[[], bool]":
