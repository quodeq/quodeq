...
| Key | Value |
| --- | --- |
| SEVERITY | Weight sliders for critical, major, and minor violation types. A type is one requirement code per principle, not one finding; the model's `vt` tag only groups findings that carry no requirement code. A readout shows how much a critical finding currently weighs relative to a minor one. |
