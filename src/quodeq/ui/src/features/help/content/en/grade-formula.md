...
| Key | Value |
| --- | --- |
| SEVERITY | Weight sliders for critical, major, and minor violation types. A type is one requirement code per principle (or one `vt` tag when present), not one finding. A readout shows how much a critical finding currently weighs relative to a minor one. |
