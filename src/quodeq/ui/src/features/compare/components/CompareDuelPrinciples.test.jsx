...
describe('CompareDuelPrinciples', () => {
  it('repeats the matching dimension scores in a group heading, none when that dimension is absent', () => {
    ...
    const { container } = render(
      <CompareDuelPrinciples principles={principles} dimensions={dimensions} aName="Alpha" bName="Beta" />,
    );
    const groups = container.querySelectorAll('.compare-duel-principles__group');
    expect(groups).toHaveLength(2);
    const [sec, use] = groups;
    // Each score is the visible digits plus the sr-only "<project>: <score>"
    // that names its side (#6370); the gap stays a bare signed number.
    expect(sec.querySelector('.compare-duel-principles__dimScores').textContent)
      .toBe('7.0Alpha: 7.05.5Beta: 5.5+1.5');
  ...
  it('falls back to the no-shared message without groups', () => {
    const { container } = render(
      <CompareDuelPrinciples principles={[]} dimensions={[]} aName="Alpha" bName="Beta" />,
    );
