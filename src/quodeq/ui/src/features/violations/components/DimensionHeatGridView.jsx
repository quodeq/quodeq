...
function buildRows(dimensions, sortCol, sortDir) {
  // Build dimension groups with their principles
  const groups = [];
  for (const dim of dimensions) {
    const violations = dim.violations || [];
    const compliance = dim.compliance || [];
    if (violations.length === 0 && compliance.length === 0) continue;
    ...
    for (const v of violations) {
      ...
      if (!principleMap.has(pName)) {
        principleMap.set(pName, { violations: 0, compliance: 0, severity: { critical: 0, major: 0, minor: 0 }, violationItems: [], complianceItems: [] });
      }
    ...
    const dimRow = {
      ...
      severity: dimSev,
      complianceRate: dimTotal > 0 ? compliance.length / dimTotal : 0,
      raw: dim,
    };
      ...
      .map(([name, data]) => {
        ...
        return {
          ...
          principleObj: {
            ...
            minor: data.severity.minor,
            violations: data.violationItems,
            compliance: data.complianceItems,
          },
        };
      });

    groups.push({ dimRow, principles });
  }
  ...
  // Sort principles within each group, then flatten
  const rows = [];
