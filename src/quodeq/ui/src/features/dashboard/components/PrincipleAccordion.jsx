...
function AccordionContent({ principle, onViolationClick, handleViolationKeyDown, contentId }) {
  return (
...
export default function PrincipleAccordion({ principle, onViolationClick }) {
  ...
  return (
    <div className="principle-accordion">
      <button className="principle-accordion-header" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls={contentId}>
      ...
      {open && <AccordionContent principle={principle} onViolationClick={onViolationClick} handleViolationKeyDown={handleViolationKeyDown} contentId={contentId} />}
    </div>
  );
}
