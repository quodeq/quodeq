// ---------------------------------------------------------------------------
// Sidebar icon constants and navigation items
// ---------------------------------------------------------------------------
import Icon from '../components/Icon.jsx';

// Sidebar rail glyphs; the star affordances and the folder chip are drawn a
// step smaller so they sit inside a row of text rather than beside it.
const NAV_ICON_SIZE = 18;
const STAR_ICON_SIZE = 16;
const FOLDER_ICON_SIZE = 14;

export const ICON_OVERVIEW = (
  <Icon size={NAV_ICON_SIZE}>
    <rect x="3" y="13" width="4" height="8" />
    <rect x="10" y="8" width="4" height="13" />
    <rect x="17" y="3" width="4" height="18" />
  </Icon>
);

export const ICON_EVALUATE = (
  <Icon size={NAV_ICON_SIZE}>
    <circle cx="11" cy="11" r="7" />
    <line x1="16.5" y1="16.5" x2="21" y2="21" />
    <line x1="8" y1="11" x2="14" y2="11" />
    <line x1="11" y1="8" x2="11" y2="14" />
  </Icon>
);

export const ICON_PROJECTS = (
  <Icon size={NAV_ICON_SIZE}>
    <polyline points="3,4 5.5,6 3,8" />
    <line x1="9" y1="6" x2="21" y2="6" />
    <polyline points="3,11 5.5,13 3,15" />
    <line x1="9" y1="13" x2="21" y2="13" />
    <polyline points="3,18 5.5,20 3,22" />
    <line x1="9" y1="20" x2="21" y2="20" />
  </Icon>
);

export const ICON_HISTORY = (
  <Icon size={NAV_ICON_SIZE}>
    <circle cx="12" cy="12" r="10" />
    <polyline points="12 6 12 12 16 14" />
  </Icon>
);

export const ICON_VIOLATIONS = (
  <Icon size={NAV_ICON_SIZE}>
    <path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
    <line x1="12" y1="9" x2="12" y2="13" />
    <line x1="12" y1="17" x2="12.01" y2="17" />
  </Icon>
);

export const ICON_MAP = (
  <Icon size={NAV_ICON_SIZE}>
    <polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6" />
    <line x1="8" y1="2" x2="8" y2="18" />
    <line x1="16" y1="6" x2="16" y2="22" />
  </Icon>
);

export const ICON_SETTINGS = (
  <Icon size={NAV_ICON_SIZE}>
    <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z" />
    <circle cx="12" cy="12" r="3" />
  </Icon>
);

export const ICON_HELP = (
  <Icon size={NAV_ICON_SIZE}>
    <circle cx="12" cy="12" r="10" />
    <path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" />
    <line x1="12" y1="17" x2="12.01" y2="17" />
  </Icon>
);

// Balance scales — weighing projects against each other.
export const ICON_COMPARE = (
  <Icon size={NAV_ICON_SIZE}>
    <line x1="12" y1="4" x2="12" y2="20" />
    <line x1="5" y1="7" x2="19" y2="7" />
    <path d="M5 7 2.5 13h5L5 7Z" />
    <path d="M19 7l-2.5 6h5L19 7Z" />
    <line x1="9" y1="20" x2="15" y2="20" />
  </Icon>
);

// The five-point star every "standard" affordance draws, at 24x24.
const STAR_OUTLINE_PATH = 'M12 2l3 7h7l-5.5 4 2 7L12 16l-6.5 4 2-7L2 9h7l3-7z';

export const ICON_STANDARDS = (
  <Icon size={NAV_ICON_SIZE}>
    <path d={STAR_OUTLINE_PATH} />
  </Icon>
);

export const ICON_STAR_OUTLINE = (
  <Icon size={STAR_ICON_SIZE}>
    <path d={STAR_OUTLINE_PATH} />
  </Icon>
);

export const ICON_STAR_FILLED = (
  <Icon size={STAR_ICON_SIZE} fill="currentColor">
    <path d={STAR_OUTLINE_PATH} />
  </Icon>
);

export const ICON_FOLDER = (
  <Icon size={FOLDER_ICON_SIZE}>
    <path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z" />
  </Icon>
);
