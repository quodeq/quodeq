...
// and over-complex functions in the UI.
//
...
// 2026-09: raised from 147 when the complexity rule (M-MOD-1) joined the
// gate with 32 grandfathered functions; max-params and max-depth carry zero.
const TOTAL_CEILING = 172; // set to the total printed by --update; lower it as entries burn down

main({
  script: 'check_hygiene.mjs',
  configPath: 'eslint.hygiene.config.js',
  baselinePath: 'tools/hygiene_baseline.json',
  rules: ['no-unused-vars', 'max-params', 'max-depth', 'complexity', 'no-restricted-syntax'],
  ...
  hint: 'Delete the unused binding (prefix with _ if it must stay), pass an options object instead of more than 6 parameters, or extract the nested/branching logic into named functions (max depth 4, max complexity 15). Run `npx eslint --no-inline-config -c eslint.hygiene.config.js <file>` for details.',
