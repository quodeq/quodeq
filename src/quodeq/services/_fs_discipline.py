...
Split out of _fs_metadata.py. ``project_has_fingerprints`` logs via the
...
def read_language_stats(reports_root: Path, entry_name: str, runs: list[RunInfo]) -> dict[str, int]:
    ...
    for run in runs:
        run_dir = reports_root / entry_name / run.run_id
...
def find_discipline_in_run(evidence_dir: Path) -> str | None:
    ...
    for ev in safe_read_dir(evidence_dir):
        if ev.name.endswith("_evidence.json"):
            found = read_discipline_from_eval(Path(ev.path))
            if found:
                return found
    return None


def infer_discipline(reports_root: Path, project: str) -> str | None:
...
def project_has_fingerprints(reports_root: Path, project: str) -> bool:
    ...
    try:
        ...
        from quodeq.services._fs_metadata import logger

        logger.warning("Could not read fingerprint dir %s: %s", project_dir, e)
    return False
