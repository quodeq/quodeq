...
class StaleDimState:
    ...
    stale_dim_map: dict[str, DimensionResult] = field(default_factory=dict)
    non_na_count: dict[str, int] = field(default_factory=dict)
    stale_previous_by_dimension: dict[str, DimensionResult] = field(default_factory=dict)
...
def record_stale_entry(entry: dict[str, Any], state: StaleDimState) -> None:
    ...
    if dim_name not in state.stale_dim_map:
        state.stale_dim_map[dim_name] = replace(
            entry["dim"],
            stale=True,
            from_run_id=entry["run_id"],
            from_date_iso=entry["date_iso"],
            from_date_label=entry["date_label"],
        )


def track_stale_grade(entry: dict[str, Any], state: StaleDimState) -> None:
...
def collect_stale_dimensions(
    ...
    get_run_dimensions: Callable[[str], list[DimensionResult]],
) -> tuple[list[DimensionResult], dict[str, DimensionResult]]:
    ...
    stale_dimensions = sorted(state.stale_dim_map.values(), key=lambda d: d.dimension or "")
