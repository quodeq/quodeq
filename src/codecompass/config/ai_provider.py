...
from codecompass.logging import log_error, log_info, log_success
...
def _write_env(paths: ConfigPaths, provider: str, api_key_var: str, api_key_value: str) -> None:
    lines = [
        "# CodeCompass provider config (generated by codecompass configure --ai-cli)",
...
def _ensure_gitignore(paths: ConfigPaths) -> None:
    ...
    if ".codecompass.env" not in content:
        content.append(".codecompass.env")
        paths.gitignore_file.write_text("\n".join(content) + "\n")
        log_info("Added .codecompass.env to .gitignore")
...
def configure_provider_noninteractive(provider: str, paths: ConfigPaths) -> int:
    ...
    log_success(f"Provider set to '{provider}'. Config saved to .codecompass.env")
    return 0
