...
by construction and are never violations.
...
def _quodeq_segments(dotted: str) -> list[str] | None:
...
def _dir_for_segments(src_root: Path, segments: Sequence[str]) -> Path:
...
def _resolve_module_dir(src_root: Path, segments: Sequence[str]) -> Path:
    ...
    if not segments:
        return src_root
    pkg_dir = _dir_for_segments(src_root, segments)
    if pkg_dir.is_dir():
        return pkg_dir
    return _dir_for_segments(src_root, segments[:-1])


def _submodule_exists(src_root: Path, segments: Sequence[str], name: str) -> bool:
...
def _classify_from_import(
    ...
    if is_private(imported_name):
        if _submodule_exists(src_root, segments, imported_name):
            return _Classification("private-module", _dir_for_segments(src_root, segments))
        return _Classification("private-name", _resolve_module_dir(src_root, segments))
    index = _first_private_index(segments)
    if index is None:
        return None
    return _Classification("private-module", _dir_for_segments(src_root, segments[:index]))
...
def _source_line(ctx: _FileCtx, lineno: int) -> str:
    return ctx.source_lines[lineno - 1].strip() if lineno <= len(ctx.source_lines) else ""


def _make_hit(ctx: _FileCtx, line: int, cls: _Classification, module: str, name: str) -> Hit:
    key = f"{ctx.rel}:{line}:{cls.kind}:{module}.{name}"
    return Hit(path=ctx.path, line=line, kind=cls.kind, module=module, name=name, key=key,
               source=_source_line(ctx, line))


def _from_import_hits(node: ast.ImportFrom, ctx: _FileCtx, src_root: Path) -> Iterator[Hit]:
    if node.level or node.module is None:
        return  # relative import: same-package by construction, never a violation
    segments = _quodeq_segments(node.module)
    if segments is None:
        return
    for alias in node.names:
        ...
        if cls is None or ctx.importer_dir == cls.required_dir:
            continue
        yield _make_hit(ctx, node.lineno, cls, node.module, alias.name)


def _plain_import_hits(node: ast.Import, ctx: _FileCtx, src_root: Path) -> Iterator[Hit]:
    for alias in node.names:
        segments = _quodeq_segments(alias.name)
        if segments is None:
            continue
        ...
        yield _make_hit(ctx, node.lineno, cls, alias.name, name)


def _file_hits(path: Path, rel: str, src_root: Path, text: str) -> list[Hit]:
    """Return every rule A/B violation in one already-read source file."""
    try:
        tree = ast.parse(text, filename=str(path))
    except SyntaxError:
        return []
    ctx = _FileCtx(path=path, rel=rel, importer_dir=path.parent, source_lines=text.splitlines())
    found: list[Hit] = []
...
def scan_tree(root: Path, src_root: Path, repo_root: Path) -> list[Hit]:
    ...
    for py in _ratchet.iter_python_files(root):
        ...
        hits.extend(_file_hits(py, rel, src_root, text))
